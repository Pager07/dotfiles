...
# /// script
# requires-python = ">=3.8"
# dependencies = ["devtools>=0.12"]
# ///
"""
Outputs a list of branches that then give you a choice of which one to checkout by index.

Run via the `gb` alias, which uses `uv run --script` so devtools is provisioned automatically.
Depends on the `git branchmin` alias (see gitconfig in this folder).
"""
