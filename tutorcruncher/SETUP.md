...
## Notes / caveats
- **Other aliases need scripts not yet migrated.** `gb`, `pt`, `lint`/`litn`, and
  `clean-local-branches` point at `~/repos/branch_checkout.py`, `run_test.py`,
  `run_linter.py`, `clean_branches.sh` — those aren't in this folder yet. They'll error
  until copied over. (Tell the user if they want these too.)
